...
to-report fittness_function_2 [x y]
  set x x / max-x * 512
  set y y / max-y * 512
  report (- x * sin(sqrt(abs x)) - y * sin(sqrt(abs y)) + 418.9829 * 2)
end
...
to-report fittness_function_3 [x y]
  ...
  foreach (range 1 5) [
    [i] ->
    set sum1 sum1 + (i * cos((i + 1) * x + i))
    set sum2 sum2 + (i * cos((i + 1) * y + i))
  ]
...
to-report fittness_function_4 [x y]
  ...
  let numerator ((sin(x ^ 2 - y ^ 2)) ^ 2 - 0.5)
  let denominator ((1 + 0.001 * (x ^ 2 + y ^ 2)) ^ 2)
  report (0.5 + (numerator / denominator))
end
...
to-report fittness_function_5 [x y]
  ...
  report (-1 * (y + 47) * sin(sqrt(abs(x / 2 + y + 47))) - x * sin(sqrt(abs(x - (y + 47)))))
end
